...
const createBooth = async (boothDetails) => {
  try {
    ...
    let checkBoothExist = await boothSchema.findOne({ boothId: boothId }).exec();    if(checkBoothExist){
...
const updateBooth = async (boothId, updation) => {
  try {
    if (updation?.boothId) {
        ...
        if (updation?.boothId !== undefined && typeof updation?.boothId !== "string") {
...
const deleteBooth = async (boothId) => {
  try{
    return await boothSchema
      .findOneAndDelete({ boothId: boothId })
      .exec();
