...
const createUser = async (email , first_name, last_name , password) => {
  try{
    ...
    const userResponse =await axiosInstance.post(`${config.ZOOM_REST_API_BASE_URL}/users/` , {...userPayload} , {
      headers: {
        Authorization: `Bearer ${tokenData.access_token}`,
      },
    })
...
const createMeeting = async (userId) => {
  try{
    ...
    const userResponse =await axiosInstance.post(`${config.ZOOM_REST_API_BASE_URL}/users/${userId}/meetings` , {...newPayload} , {
      headers: {
        Authorization: `Bearer ${tokenData.access_token}`,
      },
    })
